...
const url =
  "https://leidos.wd5.myworkdayjobs.com/en-US/External/job/Sr-Software-Engineer_R-00159780-1";
...
async function apply(url) {
  ...
  await page.waitForSelector('div[data-automation-id="myExperiencePage"]', {
    timeout: 0,
  });
...
async function signIn(page) {
  await page
    .locator('button[data-automation-id="utilityButtonSignIn"]')
    .click();
  ...
  await page
    .locator('button[data-automation-id="signInSubmitButton"]')
    .click({ delay: 500 });
}
...
async function createAccount(page) {
  ...
  await page
    .locator('button[data-automation-id="createAccountSubmitButton"]')
    .click();
}
...
async function fillExperience(page) {
  ...
  for (const work of workexperiences) {
    ...
    if (
      !(await selectorExists(
        page,
        `div[data-automation-id="workExperience-${addedWorks}"]`,
      ))
    ) {
    ...
    await withOptSelector(
      page,
      `div[data-automation-id="workExperience-${addedWorks}"] div[data-automation-id="formField-startDate"]
    ...
    await withOptSelector(
      page,
      `div[data-automation-id="workExperience-${addedWorks}"] div[data-automation-id="formField-endDate"]
            input[data-automation-id="dateSectionMonth-input"]`,
      async (el) => {
        await (await el.waitHandle()).focus();
  ...
  if (
    ...
    const uploadElementHandle = await page.$(
      'input[data-automation-id="file-upload-input-ref"]',
    );
  ...
  if (linkedInLink) {
    ...
    if (await selectorExists(page, linkedInInput)) {
      ...
      await page
        .locator(`div[data-automation-id="websitePanelSet-${addedWebs}"] input`)
        .fill(linkedInLink);
    }
  }
  if (githubLink) {
...
async function fillVoluntaryDisclosures(page) {
  ...
  /* Next Button */
  await withOptSelector(page, nextButton, async (el) => await el.click());
}
